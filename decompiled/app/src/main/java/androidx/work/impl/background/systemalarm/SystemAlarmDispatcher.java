    ...
    public class AnonymousClass1 implements Runnable {
        ...
        public void run() {
            ...
            if (intent != null) {
                ...
                PowerManager.WakeLock wakeLockNewWakeLock = WakeLocks.newWakeLock(SystemAlarmDispatcher.this.mContext, String.format("%s (%s)", action, Integer.valueOf(intExtra)));
                try {
                    Logger.get().debug(str, String.format("Acquiring operation wake lock (%s) %s", action, wakeLockNewWakeLock), new Throwable[0]);
                    ...
                    Logger.get().debug(str, String.format("Releasing operation wake lock (%s) %s", action, wakeLockNewWakeLock), new Throwable[0]);
                    ...
                    try {
                        ...
                        Logger.get().debug(str2, String.format("Releasing operation wake lock (%s) %s", action, wakeLockNewWakeLock), new Throwable[0]);
                        wakeLockNewWakeLock.release();
                        systemAlarmDispatcher = SystemAlarmDispatcher.this;
                        dequeueAndCheckForCompletion = new DequeueAndCheckForCompletion(systemAlarmDispatcher);
                    } catch (Throwable th2) {
                        Logger.get().debug(SystemAlarmDispatcher.TAG, String.format("Releasing operation wake lock (%s) %s", action, wakeLockNewWakeLock), new Throwable[0]);
                        wakeLockNewWakeLock.release();
