...
/* loaded from: discord:com/zwitserloot/cmdreader/CmdReader.SCL.lombok */
public class CmdReader<T> {
    ...
    static /* synthetic */ List access$000(CmdReader x0) {
        return x0.items;
    }
    ...
    */
            ...
            if (in[i].startsWith("--")) {
                ...
                while (true) {
                    ...
                    if (item.getFullName().equalsIgnoreCase(key)) {
                        ...
                        state.handle(item, (item.isParameterized() || value.length() != 0) ? value : null);
                        handled = true;
                    }
                }
                if (!handled) {
                ...
                if (this.seqList.size() < seq) {
                    ...
                    state.handle(this.seqList.get(seq - 1), in[i]);
                }
            }
            i++;
        }
    ...
    /* loaded from: discord:com/zwitserloot/cmdreader/CmdReader$1State.SCL.lombok */
    class C1State {
        ...
        private void checkForGlobalMandatories() throws InvalidCommandLineException {
            for (ParseItem item : CmdReader.access$000(CmdReader.this)) {
            ...
            for (ParseItem item : CmdReader.access$000(CmdReader.this)) {
                if (this.used.contains(item)) {
                    for (String n : item.getExcludes()) {
                        for (ParseItem i : CmdReader.access$000(CmdReader.this)) {
