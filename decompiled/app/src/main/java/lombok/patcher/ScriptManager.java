...
/* loaded from: discord:lombok/patcher/ScriptManager.SCL.lombok */
public class ScriptManager {
    ...
    /* loaded from: discord:lombok/patcher/ScriptManager$WitnessAction.SCL.lombok */
    ...
    static /* synthetic */ List access$1(ScriptManager scriptManager) {
        return scriptManager.scripts;
    }

    static /* synthetic */ Map access$0(ScriptManager scriptManager) {
        return scriptManager.witnessActions;
    }

    static /* synthetic */ TransplantMapper access$3(ScriptManager scriptManager) {
        return scriptManager.transplantMapper;
    }

    static /* synthetic */ Filter access$2(ScriptManager scriptManager) {
        return scriptManager.filter;
    }
    ...
    public void reloadClasses(Instrumentation instrumentation) {
        ...
        for (PatchScript s2 : this.scripts) {
            toReload.addAll(s2.getClassesToReload());
        }
    ...
    /* loaded from: discord:lombok/patcher/ScriptManager$OurClassFileTransformer.SCL.lombok */
    private class OurClassFileTransformer implements ClassFileTransformer {
        ...
        public byte[] transform(ClassLoader loader, String className, Class<?> classBeingRedefined, ProtectionDomain protectionDomain, byte[] classfileBuffer) throws IllegalClassFormatException {
            ...
            List<WitnessAction> actions = (List) ScriptManager.access$0(ScriptManager.this).get(className);
            if (actions != null) {
                for (WitnessAction wa : actions) {
                    if (!wa.triggered) {
                        wa.triggered = true;
                        if (wa.ifWitnessRemove) {
                            ScriptManager.access$1(ScriptManager.this).remove(wa.script);
                        } else {
                            ScriptManager.access$1(ScriptManager.this).add(wa.script);
                        }
                    }
                }
            }
            if (!ScriptManager.access$2(ScriptManager.this).shouldTransform(loader, className, classBeingRedefined, protectionDomain, classfileBuffer)) {
                return null;
            }
            byte[] byteCode = classfileBuffer;
            boolean patched = false;
            for (PatchScript script : ScriptManager.access$1(ScriptManager.this)) {
                try {
                    transformed = script.patch(className, byteCode, ScriptManager.access$3(ScriptManager.this));
                } catch (Throwable t) {
